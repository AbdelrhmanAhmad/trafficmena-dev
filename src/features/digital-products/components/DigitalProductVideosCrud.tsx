...
import { z } from 'zod';
import type { DigitalProductVideo } from '@/app/api/digitalProducts';
import {
  useAddDigitalProductVideo,
  useRemoveDigitalProductVideo,
  useUpdateDigitalProductVideo,
...
type DigitalProductVideosCrudProps = {
  productId: string;
...
export function DigitalProductVideosCrud({
  productId,
  videos,
  canDelete = false,
}: DigitalProductVideosCrudProps) {
  const [mode, setMode] = useState<PanelMode>('list');
  const [editingVideo, setEditingVideo] = useState<DigitalProductVideo | null>(null);

  const addMutation = useAddDigitalProductVideo(productId);
  const updateMutation = useUpdateDigitalProductVideo(productId);
  const removeMutation = useRemoveDigitalProductVideo(productId);
  ...
  const openEdit = (video: DigitalProductVideo) => {
  ...
  const handleDelete = async (videoId: string) => {
    ...
    await removeMutation.mutateAsync(videoId);
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-neutral-600">
            Paste YouTube, Bunny CDN, or other embed links. Shown to buyers after purchase.
          </p>
        </div>
        {mode === 'list' && (
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="e.g. Product walkthrough" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
