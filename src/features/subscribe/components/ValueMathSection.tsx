...
import { PRICING, TOTAL_VALUE, VALUE_MATH_VALUES } from '../content';
...
export function ValueMathSection({ currentPrice }: ValueMathProps) {
  ...
  const labels = t('subscribe.valueMathSection.items', { returnObjects: true }) as string[];
    ...
    <section className="relative w-full overflow-hidden rounded-[28px] border border-neutral-200 bg-white p-6 shadow-[0_10px_35px_-18px_rgba(16,16,16,0.45)] sm:p-8">
      <div className="relative z-10 mx-auto max-w-3xl text-center">
        <span className="text-sm font-normal text-neutral-500">
          {t('subscribe.valueMathSection.label')}
        </span>
        <h2 className="mt-2 text-3xl font-semibold tracking-tight text-neutral-900 sm:text-4xl">
          {t('subscribe.valueMathSection.title')}
        </h2>
        <p className="mt-3 text-sm text-neutral-600">{t('subscribe.valueMathSection.subtitle')}</p>
      </div>
        ...
        <div className="rounded-2xl border border-neutral-200 bg-neutral-50 p-6">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="text-lg font-semibold text-neutral-900">
              {t('subscribe.valueMathSection.whatYouGetting')}
            </h3>
          </div>
          ...
          <div className="space-y-0">
            {labels.map((label, index) => (
              ...
              >
                <span className="text-sm text-neutral-700 pe-4">{label}</span>
                <span className="text-sm font-semibold text-neutral-900 whitespace-nowrap">
                  {VALUE_MATH_VALUES[index].toLocaleString()} EGP
                </span>
              </div>
            ))}
          </div>
          ...
          <div className="mt-4 flex items-center justify-between rounded-xl bg-gradient-to-r from-amber-100 to-amber-50 px-4 py-4">
            <span className="text-base font-bold text-neutral-900">
              {t('subscribe.valueMathSection.totalValue')}
            </span>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border-2 border-amber-200 bg-gradient-to-br from-amber-50 to-white p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-lg font-semibold text-neutral-900">
                {t('subscribe.valueMathSection.yourInvestment')}
              </h3>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between py-2">
                <span className="text-sm text-neutral-600">
                  {t('subscribe.valueMathSection.regularPrice')}
                </span>
                <span className="text-xl text-neutral-400 line-through">
                  {regularPrice.toLocaleString()} {t('subscribe.valueMathSection.egpYear')}
                </span>
              </div>
              <div className="flex items-center justify-between py-2 border-t border-amber-200">
                <span className="text-base font-semibold text-amber-700">
                  {t('subscribe.valueMathSection.foundingMemberPrice')}
                </span>
                <span className="text-lg font-bold text-amber-600">
                  {displayPrice.toLocaleString()} {t('subscribe.valueMathSection.egpYear')}
                </span>
              </div>
            </div>
            ...
            <div className="mt-6 rounded-xl bg-white/80 p-4 border border-amber-100">
              <div className="flex items-center justify-between">
                <span className="text-sm text-neutral-600">
                  {t('subscribe.valueMathSection.valueToPriceRatio')}
                </span>
                <span className="text-lg font-bold text-[#05ef62]">
                  {t('subscribe.valueMathSection.valueRatio', { ratio: valueRatio })}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl bg-neutral-900 p-6 text-center">
            <p className="text-sm text-white/70 mb-2">
              {t('subscribe.valueMathSection.monthlyBreakdown')}
            </p>
            <p className="text-3xl font-bold text-white">
              {monthlyPrice.toLocaleString()} EGP
              <span className="text-lg font-normal text-white/60">
                {t('subscribe.valueMathSection.perMonth')}
              </span>
            </p>
            <p className="mt-2 text-sm text-white/50">
              {t('subscribe.valueMathSection.approxUsdMonth')}
            </p>
          </div>
        </div>
      </div>
