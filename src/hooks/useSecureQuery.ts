...
export const useSecureQuery = (options: UseSecureQueryOptions = {}) => {
  ...
  const operationsRef = useRef<Set<Promise<any>>>(new Set());
  ...
  const safeInvalidateQueries = useCallback((queryKey: string | string[], delay = 0) => {
    const invalidate = () => {
    ...
    return new Promise((resolve, reject) => {
      ...
      const operationPromise = operation()
        .then((data) => {
          ...
          operationsRef.current.delete(operationPromise);
      ...
      // Track the operation
      operationsRef.current.add(operationPromise);
    });
  }, []);

  // Debounced invalidation for frequent operations
  const debouncedInvalidateQueries = useCallback((queryKey: string | string[]) => {
