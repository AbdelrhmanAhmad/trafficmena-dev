...
import { EGYPT_PHONE_HELPER, normalizeLocalPart, validateLocalPart } from '@/shared/utils/phone';
...
const Step3: React.FC = () => {
  ...
  const validatePhone = (): string | undefined => {
    ...
    return validateLocalPart(normalizeLocalPart(localNumber, dial), dial) ?? undefined;
  };

  const buildFullPhone = () => `+${dial}${normalizeLocalPart(localNumber, dial)}`;

  const handleLocalBlur = () => {
    const normalized = normalizeLocalPart(localNumber, dial);
    setLocalNumber(normalized);
    const egError = validateLocalPart(normalized, dial);
    setErrors(egError ? { phoneNumber: egError } : {});
  };
