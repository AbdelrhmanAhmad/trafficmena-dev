...
export function validateEventData(data: Record<string, unknown>): ValidationResult {
  ...
  if (!data.date) {
    ...
    const dateValue = typeof data.date === 'string' || typeof data.date === 'number' ? data.date : String(data.date);
  ...
  if (data.max_attendees !== null && data.max_attendees !== undefined) {
    ...
    if (isNaN(attendees) || attendees <= 0) {
      errors.push({ field: 'max_attendees', message: 'Max attendees must be a positive number' });
    }
  }
...
/**
 * Validates product form data with comprehensive server-side rules
 */
export function validateProductData(data: Record<string, unknown>): ValidationResult {
  const errors: ValidationError[] = [];
