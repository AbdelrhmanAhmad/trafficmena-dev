# Payment Gateway MVP Plan v3 (Final - Ready for Implementation)

> This is the final implementation-ready version combining:
> - **v2 plan**: Full implementation details (schema, routes, services, frontend)
> - **Deep analysis**: First-principles validation, security review, conflict fixes
> - **Environment setup**: Fawaterk staging configured and tested
...
- `server/.env` configured with `FAWATERK_API_KEY` and `FAWATERK_ENV=staging`
- `server/src/config/env.ts` Zod schema updated (BE008 DONE)
- API test successful: Visa-Mastercard (2), Fawry (3), MobileWallets (4) available
- Snapshot saved: `docs/fawaterk-setup-snapshot.md`

---

## First Principles: MVP Business Model

```
┌─────────────────────────────────────────────────────────────────────────┐
│              WHY ALL THREE PAYMENT TYPES ARE MVP-ESSENTIAL             │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  SUBSCRIPTION (Yearly)                                                   │
│  └── Creates the "subscriber" status                                    │
│      └── Enables discount pricing on events/tracks                      │
│      └── Enables FREE online events for subscribers                     │
│      └── Without this: discount system has no meaning                   │
│                                                                          │
│  EVENTS (Individual)                                                     │
│  └── Core offering - paid workshops/masterminds                         │
│      └── Online events = FREE for subscribers                           │
│      └── Offline events = Discounted for subscribers                    │
│      └── Without this: no per-event monetization                        │
│                                                                          │
│  TRACKS (Bundles)                                                        │
│  └── Group of related events = value bundle                             │
│      └── Discounted for subscribers                                     │
│      └── Without this: no bundle value proposition                      │
│                                                                          │
│  ALL THREE ARE INTERCONNECTED:                                          │
│  Subscription → enables → discounts → for Events & Tracks               │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
### Fixes Applied from Deep Analysis

| Issue | Fix |
|-------|-----|
| Missing `sql` import | Add `import { sql } from 'drizzle-orm'` in schema |
| `updated.itemName` undefined | Fetch itemName from event/track table in `processSuccessfulPayment` |
| User ownership check missing | Add `eq(payments.userId, session.user.id)` in `/api/payments/verify` |
| Track booking flow unclear | Clarified: For paid tracks, create booking record on payment success |

### Validated from v2 (No Changes Needed)

- ✅ All 3 payment types (event, track, subscription)
- ✅ Subscription system with 365-day duration
- ✅ Global subscriber discount percentage
- ✅ Online event detection (derive from `meetingLink && !location`)
- ✅ Atomic UPDATE for race condition prevention
- ✅ Unique partial index for duplicate prevention
- ✅ HMAC verification format (correct per Fawaterk docs)
- ✅ Polling approach for MVP (webhook deferred)
...
**Global `subscriberDiscountPercent`** in `platformSettings` - one setting for all items.

### Online Events for Subscribers

Subscribers get **FREE** access to online events:
...
|-----------|----------|--------|
| `tracks.priceInCents` | `schema/index.ts:148` | EXISTS - no UI yet |
| `tracks.singleBookingStart` | `schema/index.ts:144` | EXISTS |
| `tracks.singleBookingEnd` | `schema/index.ts:145` | EXISTS |
| `trackBookings.paidAt` | `schema/index.ts:190` | EXISTS - unused |
| `trackBookings.pricePaidCents` | `schema/index.ts:191` | EXISTS - unused |
| Track booking logic | `routes/api/tracks.ts` | EXISTS |
| Event registration | `routes/api/events.ts:565-677` | EXISTS |
| Email service | `services/email.ts` | EXISTS - extend for receipts |
| Settings table | `schema/index.ts:317-322` | EXISTS - add subscription config |

---

## Phase 1: Schema (10 new columns across 5 tables)

### BE001: Add event pricing field

**File:** `server/src/db/schema/index.ts`

Add to `events` table (after line 74 - guestExperts):
...
Add to `trackEvents` table (after line 168 - sortOrder):
...
Add to `eventAttendees` table (after line 93 - registeredAt):
...
```typescript
import { sql } from 'drizzle-orm';  // Add this for partial index
```
...
Add to `platformSettings` table (after line 319 - inviteOnlySignup):
...
Already implemented. `server/src/config/env.ts` has:
...
async function processSuccessfulPayment(paymentId: string) {
  ...
  return db.transaction(async (tx) => {
    // FIX: Fetch itemName from actual item (not stored on payment)
    ...
    if (updated.itemType === 'track' && updated.itemId) {
      const [track] = await tx.select().from(tracks).where(eq(tracks.id, updated.itemId));
      itemName = track?.title || 'Track';

      // Create or update track booking with payment info
    ...
    // Send receipt email (non-blocking) - Deferred for MVP
...
#### 3. Verify Endpoint (Polling - MVP Primary Flow)
...
app.post('/payments/verify', async (c) => {
  ...
  if (!invoiceId) {
    return c.json({ error: { code: 'MISSING_INVOICE', message: 'invoiceId required' } }, 400);
  }

  // FIX: Verify user ownership - CRITICAL security check
  const [payment] = await db.select().from(payments)
    .where(and(
      eq(payments.fawaterkInvoiceId, invoiceId),
      eq(payments.userId, session.user.id)  // User can only verify their own payments
    ));
  ...
  if (payment.status === 'paid') {
    return c.json({ status: 'paid', alreadyProcessed: true });
  }
  ...
  if (invoiceData.paid !== 1) {
    return c.json({ status: 'pending', fawaterkPaid: false });
  }

  // Payment confirmed - process it
  try {
    const result = await processSuccessfulPayment(payment.id);
    return c.json({ status: 'paid', ...result });
  } catch (error) {
    console.error('[verify] Processing failed:', error);
    return c.json({ error: { code: 'PROCESSING_FAILED', message: error.message } }, 500);
  }
});
```

#### 4. Webhook (Optional - Deferred for MVP)
...
## Phase 8: Email (Deferred for MVP)

### BE016: Add payment receipt email

**File:** `server/src/services/email.ts`

Deferred - add after core payment flow works.
...
| `/api/payments/webhook` | POST | None | Fawaterk webhook (deferred for MVP) |
...
| User ownership | Verify `payment.userId === session.user.id` in `/api/payments/verify` |
...
| No receipt emails | User doesn't get email confirmation | Add BE016 |
...
| User tries to verify another's payment | Rejected by ownership check |

---

## Test Plan (All Three Payment Types)

### Subscription Tests
1. **Subscribe:** Free user purchases yearly subscription → Status becomes "active"
2. **Already Subscribed:** Subscriber tries to subscribe again → Rejected with expiry date
3. **Subscription Expires:** Check subscription access after `endsAt` passes

### Event Tests
4. **Paid Event (Free User):** User pays full price → Registered
5. **Paid Event (Subscriber - Offline):** Subscriber sees discounted price → Pays → Registered
6. **Online Event (Subscriber):** Subscriber sees "FREE" → Registers without payment
7. **Already Registered:** User tries to register again → "Already registered"
8. **Event Full:** Capacity reached → "Event full" before payment

### Track Tests
9. **Paid Track (Free User):** User pays full price → Booked
10. **Paid Track (Subscriber):** Subscriber sees discounted price → Pays → Booked

### Payment Flow Tests
11. **Failed Payment:** User fails card → Redirect to failed page
12. **Abandoned Payment:** User closes browser → Payment stays "pending"
13. **Invoice Verification:** `/api/payments/verify` correctly checks Fawaterk API

---

## Acceptance Criteria (Complete MVP)

### Subscription
- [ ] Free user can purchase yearly subscription (365 days)
- [ ] Subscriber status shows in profile/header
- [ ] Duplicate subscription prevented (show existing expiry)

### Events
- [ ] Free user can purchase paid event at full price
- [ ] Subscriber gets online events FREE (derived from `meetingLink && !location`)
- [ ] Subscriber gets offline/hybrid events at discounted price (global %)

### Tracks
- [ ] Free user can purchase track bundle at full price
- [ ] Subscriber gets track at discounted price (global %)
- [ ] Single-event booking works within window (trackEvents.singlePriceInCents)

### Technical
- [ ] Polling verification works (call `getInvoiceData`, check `paid === 1`)
- [ ] Atomic UPDATE prevents race conditions
- [ ] Duplicate pending payments prevented by DB index
- [ ] User ownership verified on payment verification

### Admin
- [ ] Admin can set event prices
- [ ] Admin can set track prices
- [ ] Admin can set subscription price & discount percentage
...
**Plan:** v3 - Complete with all 3 payment types + fixes applied
**Test cards:** Available

**Next action:** Create feature branch and start Phase 1 (BE001-BE007)
