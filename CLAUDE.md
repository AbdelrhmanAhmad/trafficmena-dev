...
You are an expert in TypeScript, React, Vite, Shadcn UI, Radix UI, Tailwind CSS, and Supabase.
...
2. **✅ Invitations System** - **COMPLETE** (Modern implementation)
   - Sophisticated bulk invitation processing
   - Queue management with retry logic and exponential backoff
   - Comprehensive event tracking (sent, delivered, opened, clicked, accepted)
...
6. **❌ Subscriptions** - **REMOVED FOR MVP SCOPE**
   ...
   - Library access now relies on role-based gating only
...
- **Supabase 2.54.0** (PostgreSQL + Auth + RLS + Storage)
- **Row Level Security** policies across all tables
- **Multi-provider payment system** with MENA focus
- **Queue-based email processing** for scalable communication
...
**DO for MVP:**
- Fix the library query (30 minutes)
- Simplify signup to 2 steps
- Remove fake payment implementations
- Use plain text instead of rich editors
- Ship with basic features that work
...
4. **✉️ `/src/features/invitations/CLAUDE.md`**
   - Status: Over-engineered - 5,915 lines for 400-line feature
   - MVP Action: Remove entirely or replace with simple version
...
export class EventService {
  ...
  async createEvent(eventData: CreateEventData) {
    const { data, error } = await this.supabaseCall();
    if (error) throw AppErrorHandler.handleSupabaseError(error);
    
    // Trigger library asset linkage
    await LibraryService.getInstance().linkEventAssets(data.id);
...
- **invitation_events**: Email interaction tracking
...
**Backend Performance:**
- **Database Queries**: Properly indexed with foreign key constraints
- **Caching**: Service layer singleton pattern for consistency
- **Queue Processing**: Efficient batch processing for invitations
...
1. **Stabilize Events & Library Experiences**
   - Prioritize pagination, query fixes, and content discoverability
   - Ensure admin tooling covers current MVP scope
   - Document gaps for post-MVP roadmap

2. **Standardize Route Terminology**
   - Decide on `/events` vs `/meetups` (recommend `/events`)
   - Update all route references and navigation
   - Ensure consistent terminology in UI components

3. **Complete Library Feature Consolidation**
   - Move remaining library components to feature slice
   - Implement advanced content curation
   - Add recommendation engine based on user activity

**Medium Priority:**
1. **Add Cross-Feature Orchestration Services**
...
**Next Phase Focus:**
🎯 Complete vertical slice migration (25% remaining)
🎯 Implement performance optimizations (code splitting, caching)
🎯 Add cross-feature orchestration services
🎯 Deploy comprehensive test coverage
...
### Supabase & Security Specific
- Always use Row Level Security (RLS) policies
- Never expose anon key in production builds beyond what's necessary
- Always sanitize user inputs with DOMPurify before storage
- Use parameterized queries (Supabase handles this)
- Implement proper error handling for all Supabase calls
- Use the standardized error handling pattern from utils/errorHandling
- Never log sensitive user data or authentication tokens
