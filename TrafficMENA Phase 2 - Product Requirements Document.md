...
4. Assignment model: Admin/Owner/Manager may assign a profile to one platform user. **Only that assignee** can edit that profile's public content. Assignment, publication status, and reassignment remain Admin/Owner-only; unassigned profiles remain fully admin-managed.  
...
2. Limit channels to the three settled D8 models:  
   ...
   - **Open channel:** every authenticated member can view and post, including users and experts. Selected-role, custom-role, public, and generic subscriber-only channels are not part of this model.  
