...
describe('library recording access is ticket-aware (wiring)', () => {
  it('list filters booked premium assets through bookingGrantsRecording', async () => {
    const source = await readFile(
      new URL('../../server/src/routes/api/library.ts', import.meta.url),
      'utf8',
    );
    assert.ok(
      source.includes('bookingGrantsRecording(asset.ticketType, asset.eventFormat ?? null)'),
    );
    // The booking query joins the asset's event so its delivery format is known per asset.
    assert.ok(source.includes('eventFormat: events.eventFormat'));
  });

  it('detail checks the booking ticket against the asset format', async () => {
    const source = await readFile(
      new URL('../../server/src/routes/api/library.ts', import.meta.url),
      'utf8',
    );
    assert.ok(
      source.includes('bookingGrantsRecording(booking.ticketType, asset[0].eventFormat ?? null)'),
    );
  });
