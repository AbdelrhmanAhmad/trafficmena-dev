-- Bug #11: Add server-side validation with database constraints (corrected)

-- First update any invalid existing data
UPDATE public.meetups SET date = now() + interval '1 day' WHERE date <= now();

-- Add constraints for meetups table (excluding future date constraint for now)
